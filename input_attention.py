...
for line in tqdm(questions[0:1]):
    ...
    # 首先可视化所有的层
    for i, layer in enumerate(last_layer_attentions):
        
        resized_attention = layer
        print(resized_attention.shape)

        resized_attention = resized_attention.to(torch.float32).cpu().numpy()
        ...
        print(f"Layer {i} resized_attention shape: {resized_attention.shape}, min: {np.min(resized_attention)}, max: {np.max(resized_attention)}")
        # 可视化注意力图
        plt.figure(figsize=(8, 8))
        plt.imshow(resized_attention, cmap='jet', alpha=0.5)  #
        plt.colorbar()
        plt.title(f'Attention Map Overlay Layer {i}')
        plt.axis('off')

        attention_map_file = f"./attention_maps/{idx}_layer_{i}.png"
