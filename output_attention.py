...
for line in tqdm(questions[2500:2504]):
    ...
    assistant_token_pos = inputs['input_ids'][0].tolist().index(assistant_token_id)
    ...
    # 在第一个维度去平均
    last_layer_attentions = torch.mean(last_layer_attentions, axis=0)

    # resize成为(h,w)
    H = inputs.image_grid_thw[0][1]/2
    W = inputs.image_grid_thw[0][2]/2
    last_layer_attentions = last_layer_attentions.reshape(int(H), int(W))

    # 插值成原图大小
    target_size = image_inputs[0].size
    resized_attention = F.interpolate(
        last_layer_attentions.unsqueeze(0).unsqueeze(0),  # 添加批次和通道维度
